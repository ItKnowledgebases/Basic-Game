	...
	private void initialize() {
		...
		AttackBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				//get current damage
				int UserDamage = m.SetDamage();
				int EnemyDamage = m.SetDamage();
				...
				UpdatedText = (Name + " hit the enemy with " + UserDamage + "% \n "+
						 "the enemy hit " + Name + " with " + EnemyDamage + "%");
				...
				//Display text if there is a TIE
				if (UserHP > 0 & EnemyHP > 0) {
				} else if (UserHP <= 0 && EnemyHP <= 0) {
					UpdatedText = ("WOW it was a tie!");
					OutputText.setText(UpdatedText);
				//Enemy Won
				} else if(UserHP <= 0) {
